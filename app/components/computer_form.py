import reflex as rx
from ..states.computer_state import ComputerState


def computer_form() -> rx.Component:
    return rx.el.div(
        rx.el.h2("Add New Computer Report", class_name="text-xl font-semibold mb-4"),
        rx.el.div(
            rx.el.label(
                "Computer Number", class_name="block text-sm font-medium text-gray-700"
            ),
            rx.el.input(
                placeholder="e.g., PC-12345",
                on_change=ComputerState.set_computer_number,
                class_name="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm",
                default_value=ComputerState.computer_number,
            ),
            class_name="mb-4",
        ),
        rx.el.div(
            rx.el.div(
                rx.el.input(
                    type="checkbox",
                    on_change=ComputerState.set_has_admin_password,
                    checked=ComputerState.has_admin_password,
                    class_name="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-600",
                ),
                rx.el.label(
                    "Has Admin Password?", class_name="ml-2 block text-sm text-gray-900"
                ),
                class_name="flex items-center",
            ),
            class_name="mb-4",
        ),
        rx.cond(
            ComputerState.has_admin_password,
            rx.el.div(
                rx.el.label(
                    "Admin Password",
                    class_name="block text-sm font-medium text-gray-700",
                ),
                rx.el.input(
                    type="password",
                    placeholder="Enter admin password",
                    on_change=ComputerState.set_admin_password,
                    class_name="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm",
                    default_value=ComputerState.admin_password,
                ),
                class_name="mb-4",
            ),
        ),
        rx.el.button(
            "Add Report",
            on_click=ComputerState.add_computer,
            class_name="inline-flex justify-center rounded-md border border-transparent bg-indigo-600 py-2 px-4 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2",
        ),
        class_name="p-6 bg-white rounded-lg shadow-md mb-8",
    )